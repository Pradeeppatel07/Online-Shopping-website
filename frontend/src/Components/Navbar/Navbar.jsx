...
const Navbar = () => {
  ...
  if (token) {
    try {
      userData = jwtDecode(token); // Decode the token to extract user data
      // console.log('Decoded User Data:', userData);
    } catch (error) {
      console.error('Invalid token', error);
      // Handle the error, e.g., remove the invalid token
  ...
  return (
    <div className='navbar'>
      ...
      <div className='nav-login-cart'>
        ...
        <button onClick={()=>{localStorage.removeItem('auth-token');window.location.replace("/")} }>{userData.user.username.slice(0,8) || "User"}</button> 
        </div>
